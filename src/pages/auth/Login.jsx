...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    // SECURE 1: Validasi Pola Sisi Klien (Mencegah request sampah ke Server)
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email.trim())) {
      return setError('Format alamat surel tidak valid.');
    }
    
    // SECURE 2: Error Generik (Anti-Enumeration)
    // Jangan beri tahu jika password kurang dari 8 karakter di form login, pukul rata saja errornya.
    ...
    try {
      ...
      if (!response.ok) {
        // Tetap gunakan pesan error generik apapun respons dari backend
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <img 
            ...
            alt="Ilustrasi Platform Viba.ai" 
            style={{ 
              width: '100%', 
              height: '100%', 
              objectFit: 'cover', 
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div>
              ...
              <input 
                ...
                style={{ width: '100%', padding: '14px 16px', border: '1px solid #E2E8F0', borderRadius: '12px', fontSize: '14px', boxSizing: 'border-box', outline: 'none', backgroundColor: '#FFFFFF', color: '#0F172A', transition: 'all 0.2s', opacity: isLoading ? 0.7 : 1 }} 
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input 
                  ...
                  disabled={isLoading}
                  placeholder="••••••••"
                  style={{ width: '100%', padding: '14px 44px 14px 16px', border: '1px solid #E2E8F0', borderRadius: '12px', fontSize: '14px', boxSizing: 'border-box', outline: 'none', backgroundColor: '#FFFFFF', color: '#0F172A', transition: 'all 0.2s', fontFamily: showPassword ? 'inherit' : 'caption', letterSpacing: showPassword ? 'normal' : '2px', opacity: isLoading ? 0.7 : 1 }} 
                  onFocus={(e) => e.target.style.borderColor = '#0F172A'} 
                  onBlur={(e) => e.target.style.borderColor = '#E2E8F0'} 
                />
                <button 
                  ...
                  disabled={isLoading}
                  style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: showPassword ? '#0F172A' : '#94A3B8', padding: '4px', display: 'flex', alignItems: 'center', transition: 'color 0.2s' }}
